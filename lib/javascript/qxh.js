...
return (function(args) {
  ...
  var Qxh = function(locator, findOnlyVisible, rootElement) {
    this.qxhParts = locator.split('/');
    this.findOnlyVisible = findOnlyVisible;
    if (this.findOnlyVisible) {
      console.log("Qxh searching for visible widgets only.");
    } else {
      console.log("Qxh searching for visible and invisible widgets.");
    }
  ...
  Qxh.prototype.getQxNodeDescendants = function(node) {
    ...
    for (var l=0; l<descArr.length; l++)
    {
      ...
      if ((typeof(curr) == "object") && (curr != node) && (curr !== null)) {
        ...
        } else if (!this.findOnlyVisible || (this.findOnlyVisible && curr.getVisibility() == "visible")) {
  ...
  var locator = args[0];
  var findOnlyVisible = args[1] || true;
  var rootElement = args[2] ? args[2] : "qx.ui.root.Application";
  var qxh = new Qxh(locator, findOnlyVisible, rootElement);
